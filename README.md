...
| 先看什么 | 它证明什么 |
|---|---|
| `pnpm run proof` | 零密钥、确定性的产品证明路径，可直接看懂 Claw -> DARWIN -> ATK 的主线 |
...
两种验证路径：
- `pnpm run proof`：零密钥、确定性、适合第一次理解产品。
...
项目文档：
- [Claw Intent Handoff](docs/CLAW_INTENT_HANDOFF.md)
...
- **把 OKX Agent Trade Kit 变成可运营系统**：Claw 负责理解目标和编排，OKX ATK 负责真实市场数据、账户状态和执行能力。
...
```bash
pnpm run proof                      # 零密钥、确定性的产品证明路径
pnpm run verify                     # 真实 OKX Demo 验证（需要凭证）
pnpm run demo:guided:deterministic  # 固定夹具导览演示
